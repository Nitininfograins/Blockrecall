...
const DashNav = () => {
  ...
  useEffect(() => {
    if (cookies.walletAdddress) {
      ...
      const endStr = cookies.walletAdddress.Slice(
        -4,
        cookies.walletAdddress.length
      );
      ...
      <section className="dashNav_wrap">
        <nav className="main-header navbar navbar-expand navbar-white navbar-light">
          ...
          <ul className="navbar-nav ml-auto">
            <li
              className="nav-item mr-2  mt-1 showAddress"
              id="walletAddressShow"
              style={{ marginRight: "20px" }}
            >
              <div>
                <span>{WalletAddText}</span>
              </div>
            ...
            <li className="nav-item mr-2">
              <button
                className="btn connect btn-block buy_Btn nav_item enableEthereumButton"
                id="connectWallet"
